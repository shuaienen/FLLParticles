...
void SmokeRenderer::drawPoints(int start, int count, bool sort)
{
    ...
    if (sort)
    {
		...
		//仍然是绘制 VERTEX_ARRAY 里的东西，但是读的idx根据这个element记录的来
...
// resulted in : m_halfVectorEye and m_eyePos
...
// 视角移动到光源，算一次光线和阴影，叠加到lightBuffer
void SmokeRenderer::drawSliceLightView(int i)
{
	...
	glPushMatrix();
	glLoadIdentity();
	gluLookAt(-122.014, 0.45, -m_profilePointLocation,
		-122.014, 0.45, /*30.16055 29.5796*/-999,
		0.0, 1.0, 0.0);
	...
	glPushMatrix();
	glLoadIdentity();
	glOrtho(1.505, -1.505, 0.55,-0.55,0.001,0.02);
	//GLdouble eq [4] =  {0,0,-1,0};
	//glClipPlane(GL_CLIP_PLANE0,eq);
	//glEnable(GL_CLIP_PLANE0);
	//glOrtho(0, 3.01, 0,2.75,0.001,10.0);
	//gluOrtho2D(0, 3.01, 0,2.7);
	//gluPerspective(45.0, 1.0, 0.0001, 10.0);
	...
	/*glColor4f(m_colorAttenuation[0] * m_shadowAlpha, m_colorAttenuation[1] * m_shadowAlpha, m_colorAttenuation[2] * m_shadowAlpha, 1.0);
	glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_COLOR);*/
	glColor4f(1.0, 1.0, 1.0, /*m_spriteAlpha*/1.0);
...
void SmokeRenderer::render()
{
    ...
    }
	{
		...
		glColor4f(0.0f, 55.0/255.0, 120.0/255.0, 0.3f);
		glMatrixMode(GL_MODELVIEW);
		glPushMatrix();
		glTranslatef(-121.64271,0.95,-31.2890642);

		glScalef(1/0.404,1/0.9524,1/0.4);
		glutSolidCube(1.8);
		glDisable(GL_BLEND);
		glPopMatrix();
		glDisable(GL_DEPTH_TEST);
...
void SmokeRenderer::drawProfile()
{
	...
	glVertex3f(-120.509f, 1.0f, -m_profilePointLocation);
	//glColor4f(1.0f, 1.0f, 1.0f, 0.0f);
	glVertex3f(-123.519f, 1.0f, -m_profilePointLocation);
	//glColor4f(1.0f, 1.0f, 1.0f, 0.0f);
	glVertex3f(-123.519f, -0.1f, -m_profilePointLocation);
	//glColor4f(1.0f, 1.0f, 1.0f, 0.0f);
	glVertex3f(-120.509f, -0.1f, -m_profilePointLocation);
	glEnd();
