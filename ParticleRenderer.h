...
class SmokeRenderer
{
    public:
		...
		void setDisplayProfile(bool b)
		{
			m_displayProfile = b;
		}
		void setProfileLocation(float f)
		{
			m_profilePointLocation = f;
		}
    ...
    private:
		...
		bool                m_displayProfile;
		float				m_profilePointLocation;
		...
		FramebufferObject   *m_lightFbo;//与m_lightTexture绑定

        GLuint              m_imageTex, m_depthTex;
        FramebufferObject   *m_imageFbo;//与m_imageTex绑定，最后所有slice合成到imageTex里
};
